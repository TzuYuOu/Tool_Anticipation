...
from networks import LSTMAlexNet, LSTMResNet, TimeLSTMAlexNet
...
class AnticipationModel:
	...
	def init_op(self,mode):
		...
		if mode not in ['train','test']:
			raise NotImplementedError('<mode> has to be either "train" or "test". got "{}".'.format(mode))
	...
	def forward(self,data,hidden_state):
		...
		output_cls = output[:,:-self.opts.num_ins].view(-1,self.opts.num_class,self.opts.num_ins)

		return output_reg, output_cls, hidden_state

	def compute_loss(self,output_reg,output_cls,target_reg,target_cls):

		return (self.criterion_reg(output_reg,target_reg) + self.opts.loss_scale*self.criterion_cls(output_cls,target_cls)) * self.opts.num_ins
	...
	def update_stats(self,loss,output_reg,output_cls,target_reg,target_cls,mode):
		...
		if mode == 'train':
			...
			self.train_reg += (self.eval_metric(output_reg,target_reg) / self.opts.num_ins).item()
			...
			self.test_reg += (self.eval_metric(output_reg,target_reg) / self.opts.num_ins).item()
			_, predicted = torch.max(output_cls, dim=1)
	...
	def summary(self,log_file,epoch):
		...
		if epoch % self.opts.model_save_freq == 0:
			self.net.save(os.path.join(self.model_folder,"model{:3d}.pkl".format(epoch)))
	...
	def sample_op_predictions(self,op,num_samples):
		...
		# scale regression values by <horizon>, since the network outputs value from 0 to 1
		samples_reg *= self.opts.horizon
		gt_reg *= self.opts.horizon
		# apply softmax to classification activations
		samples_cls = nn.functional.softmax(samples_cls,dim=2)
	...
	def save_samples(self,samples,ID=0,epoch=0,result_folder=None):
		...
		np.save('{}/sample_epoch_{}_op_{}_pred_reg.npy'.format(result_folder,epoch,ID),samples_reg.cpu().numpy())
		np.save('{}/sample_epoch_{}_op_{}_gt_reg.npy'.format(result_folder,epoch,ID),gt_reg.cpu().numpy())
		np.save('{}/sample_epoch_{}_op_{}_pred_cls.npy'.format(result_folder,epoch,ID),samples_cls.cpu().numpy())
		np.save('{}/sample_epoch_{}_op_{}_gt_cls.npy'.format(result_folder,epoch,ID),gt_cls.cpu().numpy())
...
class TimeModel:
	def __init__(self,opts,train=True,pretrain=None):
		...
		output_size = opts.num_ins*opts.num_class + opts.num_ins
		if opts.model == 'alexnet':
			self.net = TimeLSTMAlexNet(output_size,pretrain=pretrain).cuda()
		else:
			raise NotImplementedError('model is not implemented')
	...
	def forward(self,data, t_elasped):

		output = self.net(data, t_elasped)
		output_reg = output[:,-self.opts.num_ins:]
		output_cls = output[:,:-self.opts.num_ins].view(-1,self.opts.num_class,self.opts.num_ins)
